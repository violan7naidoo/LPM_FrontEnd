...
export default function Home() {
  ...
  useEffect(() => {
    const preloadImages = async () => {
      ...
      // Preload Background1 images (121 images)
      for (let i = 1; i <= 121; i++) {
        const img = document.createElement('img');
        const promise = new Promise<void>((resolve) => {
          img.onload = () => resolve();
          img.onerror = () => resolve();
          img.src = `/images/backround/Background1/Background1/Background1_${i}.webp`;
        });
        imagePromises.push(promise);
      }
      
      // Preload Background2 images (121 images) for free spins
      for (let i = 1; i <= 121; i++) {
        const img = document.createElement('img');
        const promise = new Promise<void>((resolve) => {
          img.onload = () => resolve();
          img.onerror = () => resolve();
          img.src = `/images/backround/Background2/Background2/Background2_${i}.webp`;
        });
        imagePromises.push(promise);
      }
